...
class Colors(RenderCVBaseModelWithoutExtraKeys):
    ...
    custom: pydantic_color.Color = pydantic.Field(
        default=pydantic_color.Color("rgb(0, 100, 20)"),
        description=
            "The color of the text enclosed by == == in yaml."
            + color_common_description
        ,
        examples=color_common_examples,
    )
...
class EntryBase(RenderCVBaseModelWithExtraKeys):
    first_column_width: TypstDimension = pydantic.Field(
        default="1fr",
    ...
    last_column_width: TypstDimension = pydantic.Field(
        default="4.15cm",
        description=length_common_description,
    )
    second_row_template: str = pydantic.Field(
        default="SUMMARY \n HIGHLIGHTS",
        title="Second Row",
        description=(
...
class PublicationEntryOptions(EntryBase):
    first_row_template: str = pydantic.Field(
        default="**TITLE** || DATE",
        title="First Row",
        description=(
...
class EducationEntryBase(RenderCVBaseModelWithoutExtraKeys):
    ...
    first_column_width: str = pydantic.Field(
        default="1cm",
